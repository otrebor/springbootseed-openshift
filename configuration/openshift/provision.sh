...
function usage() {
    ...
    echo "   --application-name [name]  Optional    name of the example application to deploy"
    echo "   --application-repo-url [url]  Optional   url of the git repository where the source code of the application is situated"
...
#sets the current directory to the actual one (needed to find templates)

EXEC_PATH=$(dirname $(realpath $0))

ARG_USERNAME=
ARG_PROJECT_SUFFIX=webapp1
ARG_APPLICATION_NAME=springbootseed-openshift
ARG_APPLICATION_REPO_URL=https://otrebor@bitbucket.org/otrebor/springbootseed-openshift.git
ARG_APPLICATION_REPO_BRANCH=master
ARG_HEALTH_CHECK_ENDPOINT_URL=/healthcheck
ARG_COMMAND=
ARG_EPHEMERAL=false
ARG_OC_OPS=
...
while :; do
    case $1 in
        ...
        --application-name)
            if [ -n "$2" ]; then
                 ARG_APPLICATION_NAME=$2
                 shift
            else
                 printf 'ERROR: "--application-name" requires a non-empty value.\n' >&2
                 usage
                 exit 255
            fi
            ;;
        --application-repo-url)
            if [ -n "$2" ]; then
                 ARG_APPLICATION_REPO_URL=$2
                 shift
            else
                 printf 'ERROR: "--application-repo-url" requires a non-empty value.\n' >&2
                 usage
                 exit 255
            fi
            ;;
...
function deploy() {
  oc $ARG_OC_OPS new-project dev-$PRJ_SUFFIX   --display-name="$PRJ_SUFFIX  - Dev"
  oc $ARG_OC_OPS new-project stage-$PRJ_SUFFIX --display-name="$PRJ_SUFFIX  - Stage"
  oc $ARG_OC_OPS new-project cicd  --display-name="CI/CD"

  sleep 2

  oc $ARG_OC_OPS policy add-role-to-group edit system:serviceaccounts:cicd -n dev-$PRJ_SUFFIX
  oc $ARG_OC_OPS policy add-role-to-group edit system:serviceaccounts:cicd -n stage-$PRJ_SUFFIX
  ...
  local templatecicd="$EXEC_PATH/templates/cicd-template-openshift.yaml"
  echo "Using template $templatecicd"
  oc $ARG_OC_OPS new-app -f $templatecicd -p DEV_PROJECT=dev-$PRJ_SUFFIX -p STAGE_PROJECT=stage-$PRJ_SUFFIX -p DEPLOY_CHE=$ARG_DEPLOY_CHE -p EPHEMERAL=$ARG_EPHEMERAL -p ENABLE_QUAY=$ARG_ENABLE_QUAY -p QUAY_USERNAME=$ARG_QUAY_USER -p QUAY_PASSWORD=$ARG_QUAY_PASS -p APPLICATION_NAME=$ARG_APPLICATION_NAME -p APPLICATION_REPO_URL=$ARG_APPLICATION_REPO_URL -p HEALTH_CHECK_ENDPOINT_URL=$ARG_HEALTH_CHECK_ENDPOINT_URL -n cicd
